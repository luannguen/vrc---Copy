...
export function formatAdminResponse(data: any, status: number = 200): NextResponse {
  ...
  headers.append('X-Payload-Refresh', 'tools');
...
export function formatBulkResponse(
  ...
  headers.append('Pragma', 'no-cache');
  headers.append('Expires', '0');
  headers.append('X-Reload-Collection', 'tools');
  ...
  return NextResponse.json(
    {
      ...
      message: `Đã xử lý ${results.length}/${results.length + errors.length} công cụ`,
      results,
...
export function handleOptionsRequest(
  ...
  // Add allowed methods to CORS headers
